...
它的智识内容并不主要是审美性的，尽管视觉设计也是其中一部分。它的基础是认知性的：人的工作记忆有限；感知依赖模式识别，而设计选择既可以利用这种能力，也可以挫败这种能力；注意力是选择性的，也可能被误导；心智模型会围绕界面呈现出的结构形成，并且会以可预测的方式偏离系统真实行为。好的界面设计符合人类认知实际运行的方式；坏的界面设计则与之对抗。这门学科发展出了一套方法——用户研究、可用性测试、启发式评估、可访问性审查——用来在用户在生产环境中发现错位之前，先发现这些错位。
...
可访问性是这样一种设计要求：界面应当能被残障人士使用。由 W3C 发布的 Web Content Accessibility Guidelines（WCAG）定义了三级可访问性符合标准，并规定了视觉、听觉、运动和认知可访问性要求。WCAG 2.1 Level AA 是大多数可访问性法律所要求的标准，也是多数生产系统的目标。

WCAG 的四项原则概括了可访问性要求：Perceivable（内容能被所有用户感知）、Operable（界面能被所有用户操作）、Understandable（内容和操作可以被理解）以及 Robust（内容能被辅助技术可靠解释）。实际要求包括：为视觉障碍提供足够色彩对比度；为无法使用指点设备的用户提供键盘可导航性；为盲人用户提供屏幕阅读器兼容性（语义化 HTML、ARIA 属性、图片替代文本）；为视频提供字幕；为非文本内容提供文本替代。

可访问性并不主要是为少数残障用户提供的功能——它惠及每个人。高对比度设计在强日光下更好用。字幕能帮助身处嘈杂环境的观看者。大触控目标既帮助有震颤的用户，也帮助手冷的用户。这就是路缘坡效应（curb-cut effect）：为残障用户设计的可访问性功能，往往会持续改善所有用户的体验。
...
第三个变化，是把可访问性当作一等要求。学过可访问性、亲眼看过屏幕阅读器如何浏览结构糟糕页面的实践者，再也不会在设计时不考虑语义化 HTML、色彩对比度和键盘导航。法律要求当然有帮助，但真正产生可访问界面的，是那种为完整用户范围设计的内化承诺。
...
对于可访问性，**WCAG 文档**（可在 <[www.w3.org/TR/WCAG21](http://www.w3.org/TR/WCAG21)> 免费获取）是权威参考。Pickering 的 **Inclusive Components**（可在 inclusive-components.design 免费获取）提供了构建可访问界面的组件级模式。
...
| Pickering, *Inclusive Components*（免费）                          | 可访问组件模式      | 入门 |
...
**Interaction Design Foundation**（IDF，订阅制，ixdf.org）提供最完整的 HCI 和 UX 设计在线课程体系——包括用户研究、交互设计、UX 写作、可访问性等。其深度超过多数大学课程，并且材料定期更新。
...
**屏幕阅读器**——VoiceOver（macOS，免费）、NVDA（Windows，免费）、TalkBack（Android，免费）——是视觉障碍用户的主要辅助技术。让屏幕阅读器运行在你构建的界面上，并尝试在不看屏幕的情况下使用它，是理解可访问性实践含义的最直接方式。这种体验通常会令人谦卑，也很有生产力。

**axe DevTools 浏览器扩展**（免费）和 **WAVE**（免费）是自动可访问性检查工具。用它们检查任何界面，都会揭示最常见的可访问性违规。它们不能发现所有问题——自动工具大约只能捕捉 30% 的可访问性问题——但它们可以在用户测试之前先抓住容易发现的问题。
...
| 事后补可访问性       | 事后修补可访问性远比一开始就为可访问性设计更昂贵，而且修补通常不完整。一个建立在语义化、结构良好的 HTML、正确 ARIA 属性和足够色彩对比度上的界面，默认就更可访问；一个没有这些脚手架的界面，要变得可访问，就需要大量返工。                   | 把可访问性整合进设计流程。在设计审查中包含可访问性标准。在视觉设计阶段检查色彩对比，而不是之后再检查。在开发期间用屏幕阅读器测试，而不是上线前才测。可访问性是设计约束，不是后来添加的功能。                                               |
