...
SAT 和 SMT 求解器是自动推理在当代工业中的形式。SAT 问的是一个命题公式是否存在满足赋值；SMT 把这个问题扩展到更丰富的理论中，例如线性算术、数组、位向量。当软件工程师使用符号执行工具检查某个函数永远不会越界访问内存，或者硬件设计者验证电路是否正确实现了规格说明时，他们使用的就是直接源自这一传统的自动推理。
...
| Pearl, *Causality* (2nd ed.)                                             | 因果推理       | 深入 |
