...
神经网络中的 softmax 函数是数值稳定性立刻变得重要的一个案例。朴素实现中，softmax(x)_i = exp(x_i)/Σexp(x_j)，当 x_j 很大时会溢出。稳定实现会在指数运算前减去最大值：exp(x_i - max(x))/Σexp(x_j - max(x))。这不会改变数学结果，但能防止溢出。每个机器学习框架都会在内部这样实现；但理解为什么要这样做，会揭示数学规格说明与计算实现之间的关系。
