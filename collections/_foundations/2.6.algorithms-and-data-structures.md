...
NP-completeness 不是目录项，而是诊断工具。当一个新的组合优化问题顽固抵抗时——你尝试的每个算法都太慢，把它规约成更简单问题也失败——第一个正确问题应该是：这个问题是不是 NP-complete？如果是，就应当放弃寻找多项式时间精确解，转向替代方案：能以可证明保证找到近似最优解的近似算法；在输入的某个结构参数较小时高效的参数化算法；具有良好经验表现的启发式算法；或者只针对实践中实际出现的特殊情形设计的精确算法。

Karp 1972 年的论文确立了 21 个经典 NP-complete 问题——可满足性、顶点覆盖、独立集、图着色、Hamiltonian cycle、旅行商问题等——它们可以作为规约目标。识别一个新问题类似于其中某个问题，并构造能证明等价性的多项式时间规约，本身就是一种算法设计能力。越早识别难解性，就越会改变实际会被构建的东西。
...
第二个变化是规约识别能力：看出一个新问题其实是一个已知问题。一个调度问题可以规约为最大流；一个资源分配问题可以规约为二分图匹配；一个依赖问题是 DAG 遍历。识别出规约，意味着使用几十年来的已知结果，而不是从零设计。这种识别能力来自对经典算法的广泛接触，这也是为什么这门学科无法通过狭窄题集充分学会。
