...
互信息 I(X; Y) = H(X) - H(X|Y) 度量知道 Y 会减少多少关于 X 的不确定性。它是对称的——I(X;Y) = I(Y;X)——并且当且仅当 X 和 Y 独立时等于零。互信息出现在特征选择中（选择与目标具有高互信息的特征）、表示学习中（信息瓶颈原则），也出现在对神经网络注意力模式的分析中。与相关系数不同，互信息能够捕捉非线性依赖。
