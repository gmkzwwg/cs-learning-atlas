...
测试是执行软件以检查其行为是否正确的活动——这里的“正确”，指的是符合某种 specification，无论这种 specification 是形式化还是非形式化。这个定义包含一个重要限制：测试只能证明 bug 的存在，不能证明 bug 的不存在。Edsger Dijkstra 在 1972 年说过这一点，其逻辑很直接。一个程序对所有输入的正确性需要证明；一个测试只能证明它在被测试的特定输入上正确。除非输入空间有限且足够小，可以被完全穷尽，否则测试就是 sampling，而 sampling 天然存在 coverage gaps。问题不是要不要测试——测试是必要的——而是如何把测试做得足够好，使 coverage gaps 可管理，而不是灾难性的。

软件测试这门纪律为这个问题提供结构化回答：应该写哪些 test types，以什么比例写，在什么 abstraction level 写，用什么工具写，使用什么 input generation 策略，以及如何把测试整合进开发流程，使它提供持续反馈，而不是一次性 gate。过去二十年里，这些答案发生了很大变化，推动因素包括 automated testing infrastructure 的发展、test-driven development 作为实践的出现、continuous integration 作为部署模型的普及，以及 property-based testing 作为生成开发者想不到的输入的方法。
